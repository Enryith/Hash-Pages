...
<div class="btn-group mr-2">
	@auth
		<button class="btn btn-outline-dark js-vote js-agree pt-0 pb-0 {{$agree}}" data-discussion="{{ $discussion->getId() }}">
			<strong class="text-success text-lg-center js-agree-text vote">
				{{$discussion->getCachedAgree()}}
			</strong>
		</button>
	@else
		<div class="btn btn-outline-dark no-hover disabled">
	...
	<a class="btn btn-dark" href="{{ url("/tag/{$discussion->getTag()->getTag()}") }}">#{{ $discussion->getTag()->getTag() }}</a>

	@auth
		<button class="btn btn-outline-dark js-vote js-disagree pt-0 pb-0 {{$disagree}}" data-discussion="{{ $discussion->getId() }}">
